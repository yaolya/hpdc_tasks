...
void findMaxOfMinTriangular(int **matrix, int size, int threads)
{
...
int **getTriangularMatrix(int size)
{
    int **ar = new int *[size];
...
int main()
{
    ...
    for (int i = 0; i < size_cases; i++)
    {
        int **matrix = getTriangularMatrix(sizes[i]);
        for (int j = 0; j < threads_cases; j++)
        {
            omp_set_num_threads(num_threads_list[j]);
            average_time[i][j] = measureTime(findMaxOfMinTriangular, matrix, sizes[i], num_threads_list[j]);
        }
